...
│   │   └── handlers.py     # полный поток обработки сообщения
│   ├── triggers/
...
| `LLM_MODEL` | `gpt-5.4` | Модель OpenAI (`gpt-5.4-mini` — дешевле) |
...
## Доступ к OpenAI из РФ

OpenAI API не отвечает на запросы с российских IP. Варианты:

- задать `OPENAI_BASE_URL` — адрес OpenAI-совместимого прокси/шлюза;
- запускать бота там, где `api.openai.com` доступен (VPS, туннель).

Telegram Bot API от этого не зависит и работает штатно — это разные
сетевые каналы.
